...
export const VOTING_ADDRESSES = {
  GAUGE_CONTROLLER: (process.env.NEXT_PUBLIC_GAUGE_CONTROLLER_ADDRESS || '0x') as `0x${string}`,
  VOTING_ESCROW: (process.env.NEXT_PUBLIC_VOTING_ESCROW_ADDRESS || '0x') as `0x${string}`,
};
...
export const calculateVotingPower = (
  ...
  return (totalPower * BigInt(Math.floor(allocationPercentage * 100))) / BigInt(10000);
};
