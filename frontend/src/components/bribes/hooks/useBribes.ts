...
import { LIQUIDITY_POOLS, getGaugeAddress } from '../../liquidity/constants';
...
export const useBribes = () => {
  ...
  const [state, setState] = useState<BribeMarketplaceState>(BribeMarketplaceState.LOADING);
  ...
  const allBribes: Bribe[] = useMemo(() => {
    const bribes: Bribe[] = [];
    const bribeData = [bribe0, bribe1, bribe2, bribe3, bribe4, bribe5, bribe6, bribe7, bribe8, bribe9];
    ...
    for (let i = 0; i < (Number(nextBribeId) || 0) && i < 10; i++) {
      ...
      const pool = LIQUIDITY_POOLS.find((p) => {
        const gaugeAddr = getGaugeAddress(p.name);
    ...
    return bribes;
  }, [nextBribeId, bribe0, bribe1, bribe2, bribe3, bribe4, bribe5, bribe6, bribe7, bribe8, bribe9]);
  ...
  const handleCreateBribe = async (formData: CreateBribeFormData) => {
    ...
    const gauge = getGaugeAddress(formData.pool.name);
    if (!gauge) {
    ...
    try {
      ...
      // Check allowance
      const { needsApproval } = await checkAllowance(formData.token.address, formData.amount);
  ...
  const validateCreateBribe = (formData: CreateBribeFormData): ValidationResult => {
    if (!isConnected) {
  ...
  const validateClaimBribe = (formData: ClaimBribeFormData): ValidationResult => {
    if (!isConnected) {
      return { isValid: false, error: 'Please connect wallet' };
    }
   ...
   */
  const calculateBribeAPR = (bribeAmount: bigint, totalVotes: bigint): string => {
