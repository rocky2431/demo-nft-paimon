...
export function useTreasuryDeposit({
  ...
  const [txStep, setTxStep] = useState<'idle' | 'approving' | 'approved' | 'depositing' | 'completed'>('idle');

  // Parse amount
  const amountBigInt = amount && parseFloat(amount) > 0 ? parseUnits(amount, decimals) : 0n;
  ...
  const { data: approveSimulation } = useSimulateContract({
    ...
    query: {
      enabled: !!tokenAddress && amountBigInt > 0n && txStep === 'idle',
    },
  });
  ...
  const { data: depositSimulation } = useSimulateContract({
    ...
    args: tokenAddress && amountBigInt > 0n ? [tokenAddress as `0x${string}`, amountBigInt] : undefined,
    query: {
      enabled: !!tokenAddress && amountBigInt > 0n && txStep === 'approved',
    },
  });
