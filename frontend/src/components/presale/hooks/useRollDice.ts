...
const REMINT_CONTROLLER_ADDRESS = process.env.NEXT_PUBLIC_REMINT_CONTROLLER_ADDRESS as `0x${string}`;
const BOND_NFT_ADDRESS = process.env.NEXT_PUBLIC_BOND_NFT_ADDRESS as `0x${string}`;

// Minimal ABI for RemintController
const REMINT_CONTROLLER_ABI = [
  parseAbiItem('function rollDice(uint256 tokenId) external returns (uint256 requestId)'),
  parseAbiItem('function getDiceData(uint256 tokenId) external view returns (uint8 diceType, uint8 rollsThisWeek, uint256 lastRollTimestamp, uint256 totalRemintEarned, uint256 lastWeekNumber, uint8 highestDiceRoll)'),
  parseAbiItem('event DiceRolled(uint256 indexed tokenId, uint256 requestId, uint8 diceType, uint8 result, uint256 remintAmount)'),
] as const;

// Minimal ABI for Bond NFT
const BOND_NFT_ABI = [
  parseAbiItem('function balanceOf(address owner) external view returns (uint256)'),
  parseAbiItem('function tokenOfOwnerByIndex(address owner, uint256 index) external view returns (uint256)'),
] as const;
...
export function useRollDice(tokenId?: number) {
  ...
  const activeTokenId = tokenId ?? (firstTokenId ? Number(firstTokenId) : undefined);
  ...
  const { isSuccess: isRollSuccess, isLoading: isWaitingForReceipt } = useWaitForTransactionReceipt({
    hash,
  });
  ...
  useEffect(() => {
    if (isRollSuccess) {
      ...
      setTimeout(() => {
        const maxValue = parsedDiceData ? [6, 12, 20][parsedDiceData.diceType] : 6;
