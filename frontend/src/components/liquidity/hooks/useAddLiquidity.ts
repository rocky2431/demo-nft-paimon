...
export const useAddLiquidity = () => {
  ...
  const [addLiquidityState, setAddLiquidityState] = useState<AddLiquidityState>(AddLiquidityState.IDLE);
  ...
  const { data: allowanceA } = useReadContract({
    ...
    args: address ? [address, LIQUIDITY_ADDRESSES.PANCAKE_ROUTER] : undefined,
    query: { enabled: !!address && !!formData.pool },
  });
  ...
  const handleTokenAChange = useCallback(
    (amount: string) => {
      ...
      try {
        const amountParsed = amount === '' ? 0n : parseUnits(amount, formData.pool.token0.decimals);
        const [reserve0, reserve1] = reserves;

        // Calculate token B amount based on pool ratio
        const amountB = reserve0 > 0n ? quoteTokenAmount(amountParsed, reserve0, reserve1) : 0n;
        ...
        setFormData((prev) => ({
          ...
          tokenA: {
            ...
            balance: balanceA || 0n,
            balanceFormatted: formatUnits(balanceA || 0n, formData.pool!.token0.decimals),
          },
          tokenB: {
            token: formData.pool!.token1,
            amount: amountB,
            amountFormatted: formatUnits(amountB, formData.pool!.token1.decimals),
            balance: balanceB || 0n,
            balanceFormatted: formatUnits(balanceB || 0n, formData.pool!.token1.decimals),
          },
        }));
      } catch (error) {
  ...
  const approveTokenA = useCallback(async () => {
    if (!formData.tokenA || !address) return;
    ...
    try {
      ...
      const hash = await writeContractAsync({
        ...
        args: [LIQUIDITY_ADDRESSES.PANCAKE_ROUTER, formData.tokenA.amount],
      });
  ...
  }, [formData.tokenA, address, writeContractAsync]);

  /**
   * Approve Token B
   */
  const approveTokenB = useCallback(async () => {
    if (!formData.tokenB || !address) return;
    ...
    try {
      ...
      const hash = await writeContractAsync({
        ...
        abi: erc20Abi,
        functionName: 'approve',
        args: [LIQUIDITY_ADDRESSES.PANCAKE_ROUTER, formData.tokenB.amount],
      });
      await new Promise((resolve) => setTimeout(resolve, 2000)); // Wait for confirmation
    } catch (error: any) {
      setAddLiquidityState(AddLiquidityState.ERROR);
  ...
  }, [formData.tokenB, address, writeContractAsync]);

  /**
   * Add liquidity
   */
  const addLiquidity = useCallback(async () => {
    if (!formData.pool || !formData.tokenA || !formData.tokenB || !address) return;

    try {
      setAddLiquidityState(AddLiquidityState.ADDING);

      const amountAMin = calculateAmountMin(formData.tokenA.amount, formData.slippageBps);
      const amountBMin = calculateAmountMin(formData.tokenB.amount, formData.slippageBps);
      const deadline = calculateDeadline(formData.deadlineMinutes);

      const hash = await writeContractAsync({
        address: LIQUIDITY_ADDRESSES.PANCAKE_ROUTER,
        abi: ROUTER_ABI,
  ...
  }, [formData, address, writeContractAsync]);
  ...
  const preview = useMemo((): LiquidityPreview | null => {
    if (!formData.tokenA || !formData.tokenB || !reserves || !totalSupply) return null;
    ...
    const priceToken0 = formatPriceRatio(reserve0, reserve1, formData.pool!.token0.decimals, formData.pool!.token1.decimals);
    const priceToken1 = formatPriceRatio(reserve1, reserve0, formData.pool!.token1.decimals, formData.pool!.token0.decimals);
    ...
    return {
      ...
      priceToken0,
      priceToken1,
      amountAMin: calculateAmountMin(formData.tokenA.amount, formData.slippageBps),
      amountBMin: calculateAmountMin(formData.tokenB.amount, formData.slippageBps),
    };
  ...
  const validation = useMemo((): ValidationResult => {
    ...
    if (formData.tokenA.amount > (balanceA || 0n)) {
      return { isValid: false, error: VALIDATION_MESSAGES.INSUFFICIENT_BALANCE_A };
    }

    if (formData.tokenB && formData.tokenB.amount > (balanceB || 0n)) {
      return { isValid: false, error: VALIDATION_MESSAGES.INSUFFICIENT_BALANCE_B };
    }
