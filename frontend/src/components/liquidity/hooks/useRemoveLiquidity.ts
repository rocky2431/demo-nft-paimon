...
export const useRemoveLiquidity = () => {
  ...
  const [removeLiquidityState, setRemoveLiquidityState] = useState<RemoveLiquidityState>(RemoveLiquidityState.IDLE);
  ...
  const approveLPToken = useCallback(async () => {
    if (!formData.pool || !formData.lpTokens || !address) return;
    ...
    try {
      ...
      const hash = await writeContractAsync({
        ...
        args: [LIQUIDITY_ADDRESSES.PANCAKE_ROUTER, formData.lpTokens],
      });
  ...
  }, [formData, address, writeContractAsync, refetchAllowance]);

  /**
   * Remove liquidity
   */
  const removeLiquidity = useCallback(async () => {
    if (!formData.pool || !formData.lpTokens || !address || !reserves) return;

    try {
      setRemoveLiquidityState(RemoveLiquidityState.REMOVING);

      const [reserve0, reserve1] = reserves;
      const amount0 = calculateRemoveAmount(formData.lpTokens, reserve0, totalSupply || 0n);
      const amount1 = calculateRemoveAmount(formData.lpTokens, reserve1, totalSupply || 0n);
  ...
  }, [formData, address, reserves, totalSupply, writeContractAsync]);
  ...
  const preview = useMemo((): RemoveLiquidityPreview | null => {
    if (!formData.pool || !formData.lpTokens || formData.lpTokens === 0n || !reserves || !totalSupply) {
      return null;
    }

    const [reserve0, reserve1] = reserves;

    const amount0 = calculateRemoveAmount(formData.lpTokens, reserve0, totalSupply);
    const amount1 = calculateRemoveAmount(formData.lpTokens, reserve1, totalSupply);
