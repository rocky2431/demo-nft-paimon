...
import { getGaugeAddress } from '../constants';
...
export const useStaking = () => {
  ...
  const [stakingState, setStakingState] = useState<StakingState>(StakingState.IDLE);
  const [errorMessage, setErrorMessage] = useState<string>('');

  // Get gauge address
  const gaugeAddress = formData.pool ? getGaugeAddress(formData.pool.name) : null;
  ...
  }, [gaugeAddress, totalStakedData, rewardRateData, formData.pool, formData.stakedBalance, formData.earnedRewards]);
  ...
  }, [isConnected, formData.pool, formData.amount, formData.action, allowanceData]);
