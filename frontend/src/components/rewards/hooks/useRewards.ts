...
export const useRewards = () => {
  ...
  const gauge0 = getGaugeAddress(pool0.name);
  ...
  const gauge1 = getGaugeAddress(pool1.name);
  ...
  const gauge2 = getGaugeAddress(pool2.name);
  ...
  const gauge3 = getGaugeAddress(pool3.name);
  ...
  const summary: RewardsSummary = useMemo(() => {
    const totalEarnedPAIMON = poolRewards.reduce((sum, pool) => sum + pool.earnedRewards, 0n);
    const activePositions = poolRewards.filter((pool) => pool.stakedBalance > 0n).length;
  ...
  const handleClaimAll = async () => {
    const poolsWithRewards = poolRewards.filter((pool) => pool.earnedRewards > 0n);
