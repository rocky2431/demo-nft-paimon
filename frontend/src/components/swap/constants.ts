...
export const TOKEN_CONFIG: Record<Token, TokenInfo> = {
  [Token.USDC]: {
    ...
    name: 'USD Coin',
    decimals: 6,
    address: (process.env.NEXT_PUBLIC_USDC_ADDRESS || '0x') as `0x${string}`,
  ...
  [Token.HYD]: {
    ...
    name: 'Hydra Token',
    decimals: 18,
    address: (process.env.NEXT_PUBLIC_HYD_ADDRESS || '0x') as `0x${string}`,
    icon: '/tokens/hyd.svg',
  },
};
...
export const CONTRACT_ADDRESSES = {
  PSM: (process.env.NEXT_PUBLIC_PSM_ADDRESS || '0x') as `0x${string}`,
};
